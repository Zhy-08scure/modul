...
function icon() {
   $cap = json_decode(roll(host.'system/libs/captcha/request.php', h(1), "cID=0&rT=1&tM=light")[1], 1);
   foreach ($cap as $c) {
      $im[] = strlen(base64_encode(roll(host.'system/libs/captcha/request.php?cid=0&hash='.$c, h(0, 1))[1]));
   }
   $no = index($im);
   $res = roll(host.'system/libs/captcha/request.php', h(1), "cID=0&pC=".$cap[$no]."&rT=2")[1];
   return $cap[$no];
}
function CheckApi() {
...
#      z("Account Verified", '');
      ...
      return $apikey;
 #  } else {
 #     exit(x("\033[1;37mApikey: ", mc."Something wrong!".n));
#   }
}
function MenuApi() {
   wc();
