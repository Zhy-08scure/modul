...
function save($new) {
  if (file_exists($new)) {
    ...
    $cfg = readline(input($new));
    print n; file_put_contents($new, $cfg);
  }
  return $cfg;
}
function slow($str) {
...
function ban() {
  ...
  $panjang = 56-strlen($hari.$tanggal.$waktu); $spasi = floor($panjang/2);
  ...
  if ($ap) {
    ...
    cetak(pm.str_pad($ap['city'].", ".$ap['regionName'].", ".$ap['country'], 56, " ", STR_PAD_BOTH).d.n, 20000);
    gas(b.str_repeat("=", 56).n);
  } else {
    date_default_timezone_set("UTC"); return "UTC";
  }
  cetak(m.str_pad("_____         _____ _       _", 56, " ", STR_PAD_BOTH).n, 15000);
  cetak(m.str_pad("|     |_ _ ___|     |_|___ _| |", 56, " ", STR_PAD_BOTH).n, 15000);
  cetak(p.str_pad("|  |  | | |  _| | | | |   | . |", 56, " ", STR_PAD_BOTH).n, 15000);
  cetak(p.str_pad("|_____|___|_| |_|_|_|_|_|_|___|", 56, " ", STR_PAD_BOTH).n, 15000);
  print b.str_repeat(o."─", 19).p." ❝ ".h."のスクリプト".p." ❞ ".str_repeat(o."─", 19).n;
  Acssi_calvin(scr); print n;
  cetak(tb2."       \033[1;35mThanks To : ".n, 15000);
  cetak(mp.str_pad("| Reyhan H | Sugiono Official | kakatoji |", 56, " ", STR_PAD_BOTH).d.n, 15000);
  cetak(pm.str_pad("| IEWIL OFFICIAL | bintang catur | CMP |", 56, " ", STR_PAD_BOTH).d.n, 15000);
  gas(b.str_repeat("═", 19).c."元のバージョン ".ver.b.str_repeat("═", 19).n.d);
